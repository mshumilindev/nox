...
@MainActor
public final class NoxMemoryAggregator {
    public init() {}
    private let metadataExtractor = NoxMetadataExtractor()
    public private(set) var openSpan: NoxActivitySpan?
    private var lastWorkSpan: NoxActivitySpan?
    private var pendingInterruption: (from: NoxActivitySpan, at: Date)?

    public func ingestSnapshot(_ snapshot: NoxActivitySnapshot) -> NoxActivitySpan? {
    ...
    public func closeOpenSpan(at date: Date) -> NoxActivitySpan? {
    ...
    public func restoreOpenSpan(_ span: NoxActivitySpan) {
