...
nonisolated final class NoxActivityObserver: @unchecked Sendable {
    ...
    private func startTimer() {
        schedulePollTimer(relaxed: false)
    }

    private func schedulePollTimer(relaxed: Bool) {
        if relaxed == usesRelaxedPolling, timer != nil { return }
        usesRelaxedPolling = relaxed
        timer?.cancel()
        let interval = relaxed ? relaxedPollInterval : activePollInterval
    ...
    private func emitCurrentSnapshot(force: Bool) {
        ...
        let sameSurface = lastSnapshot.map { $0.hasSameObservationSurface(as: snapshot) } ?? false
        lastSnapshot = snapshot
        onSnapshot?(snapshot)
        let inBurst = burstTimer != nil
        schedulePollTimer(relaxed: !force && sameSurface && !inBurst)
    }
