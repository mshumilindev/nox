...
final class CompositePresenceDiscoveryProvider: PresenceDiscoveryProvider, @unchecked Sendable {
    ...
    func setAppleBluetoothDiscoveryActive(_ active: Bool) {
        ...
        bonjour.setAppleEcosystemBrowsingActive(active)
    }
    ...
    private func appleGroupCollapseKey(for node: NoxDiscoveredNode) -> String? {
        ...
        // HomePod stereo pairs can publish the same room through AirPlay/RAOP with different
        // transient group identifiers. The user-facing room/pair name is the stable identity.
        ...
        if !normalizedName.isEmpty {
            return "homepod-name-\(normalizedGroupKey(normalizedName))"
        }
