...
final class NoxMemoryCoordinator {
    ...
    private var lastFocusPersistFingerprint: String?
    ...
    func clearRecentActivity(from start: Date, to end: Date) async throws -> Int {
        lastFocusPersistFingerprint = nil
        return try await memoryStore.deleteSpans(inRange: start, to: end)
    }
    ...
    func loadView(
        ...
        let focusFingerprint = focusPersistFingerprint(
            period: period,
            query: query,
            spans: spans,
        ...
        var storedBlocks: [NoxFocusBlock]
        if focusFingerprint == lastFocusPersistFingerprint {
            storedBlocks = try await memoryStore.focusBlocks(from: range.start, to: range.end)
        } else {
            try await memoryStore.clearFocusBlocks(from: range.start, to: range.end)
            for block in analysis.blocks {
                try await memoryStore.insertFocusBlock(block)
            }
            lastFocusPersistFingerprint = focusFingerprint
            storedBlocks = try await memoryStore.focusBlocks(from: range.start, to: range.end)
        }
    ...
    private func focusPersistFingerprint(
        period: NoxMemoryPeriod,
        query: NoxMemoryQuery,
        spans: [NoxActivitySpan],
        interruptions: [NoxInterruption],
        blocks: [NoxFocusBlock]
    ) -> String {
        let spanSig = spans
            .sorted { $0.startedAt < $1.startedAt }
            .map { span in
                let end = Int((span.endedAt ?? span.startedAt).timeIntervalSince1970)
                return "\(span.id):\(end)"
            }
            .joined(separator: "|")
        let blockSig = blocks
            .sorted { $0.startedAt < $1.startedAt }
            .map { "\($0.id):\(Int($0.startedAt.timeIntervalSince1970)):\(Int($0.endedAt.timeIntervalSince1970))" }
            .joined(separator: "|")
        return "\(period.rawValue)|\(query.normalizedText)|\(spanSig)|\(interruptions.count)|\(blockSig)"
    }
